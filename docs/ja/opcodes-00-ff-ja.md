...
| 56h | CONFIRMED | MVL | (m), [r3±n] | 4 | 5+2×I | p.230「0101 0110 = 56h / bytes:4 / cycles:5+2×I」。operands=m, n。p.230拡大写真直読、独立した確認結果と照合一致 |
...
| 5Eh | CONFIRMED | MVL | [r3±m], (n) | 4 | 5+2×I | p.230「0101 1110 = 5Eh / bytes:4 / cycles:5+2×I」。operands=n, m。p.230拡大写真直読、独立した確認結果と照合一致 |
...
| DFh | PARTIAL | OFF | - | 1 | 〃 | p.102「DF」直読。p.230拡大「1101 1111 = DFh / bytes:1」直読。p.230命令表でHALT（DEh）cycles=「通過時2」の直下に「〃」表記。p.102個別命令説明ページにcycles記載なし。同値継承の可能性は高いが確定解釈を含むためPARTIAL維持 |
...
| FEh | PARTIAL | IR | - | 1 | 記載なし | p.102「FE」直読。p.230「1111 1110 = FEh / bytes:1」直読。p.102個別命令説明ページにcycles欄なし。命令表での当該行も未確認 |
| FFh | PARTIAL | RESET | - | 1 | 記載なし | p.103「FF」直読。p.103優先でFFh確定。p.103個別命令説明ページにcycles欄なし。命令表での当該行も未確認 |
...
**56h / 5Eh：**
CONFIRMED（v6_C）。p.230拡大写真直読でoperand順確定：56h=m, n、5Eh=n, m。独立した確認結果と照合一致。

**6Ch / 7Ch：**
INC/DEC r の第2バイト「0000 0r」形式を p.228拡大直読で確認。下位3ビットがレジスタ選択表に従いレジスタを選択。基本 opcode は確定。r 展開を明示表に完全展開していないため PARTIAL 維持。

**DFh：**
opcode / mnemonic / bytes は確認済み。p.230命令表でHALT（DEh）「通過時2」の直下に「〃」表記。p.102個別命令説明ページにcycles記載なし。同値継承の可能性は高いが確定解釈を含むためPARTIAL維持。

**FEh / FFh：**
opcode / mnemonic / bytes は確認済み。p.102/p.103個別命令説明ページにcycles欄自体なし。命令表での当該行も未確認。cycles = 記載なし。
